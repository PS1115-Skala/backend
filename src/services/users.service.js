...
class UsersService {
	...
	async getAdminUsers() {
		const query = 'SELECT * FROM usuario WHERE type = 3333';
		const requestsUsers = await pool.query(query);
		return requestsUsers || [];
	}

	async getProfesor() {
		const query = 'SELECT * FROM usuario WHERE type = 1111 or type = 2222';
	...
	async registerUser(usbId, name, email, type, chief, clave) {
		...
		const query = `INSERT into usuario (id,name, email, type, is_active,chief, clave)
	...
	async loginUser(usbId, clave) {
		const query = `SELECT id, clave, type from usuario where id='${usbId}'`;
		const login = await pool.query(query);
		const user = login.rows[0];
		if (login.rows < 1) return 0;
