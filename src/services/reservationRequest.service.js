...
class ReservationRequestService {
	async getRequest(solicitudId) {
		const query = `SELECT * FROM reservation_request WHERE id = ${solicitudId}`;
	...
	async getScheduleFromRequest(solicitudId) {
		...
		if (content.length == 1) response.typeWeek = content[0].week.toString();
		else if (content.length > 1) {
			const first = content[0];
			const second = content[1];
			if (first.hour == second.hour) {
				if (first.week % 2 == 0) {
					if (second.week % 2 == 0) response.typeWeek = 'pares';
					else response.typeWeek = 'todas';
				} else {
					if (second.week % 2 != 0) response.typeWeek = 'impares';
					else response.typeWeek = 'todas';
				}
			} else response.typeWeek = content[0].week.toString();
		}
		return response;
	}
	...
	async getRequestUser(userId) {
		...
		const query = 'SELECT * FROM reservation_request WHERE requester_id = $1 AND trimester_id = $2';
		const requestsUsers = await pool.query(query, values);
		return requestsUsers || [];
	}

	async getAllPendingRequestOfLastTrim(trimester_id) {
		const query = `SELECT id FROM reservation_request WHERE status = 'P' AND trimester_id = '${trimester_id}'`;
	...
	async updateRequest(id, reason, status) {
		const query = `UPDATE reservation_request SET reason = '${reason}', status = '${status}' WHERE id = ${id}`;
	...
	async createReservation(room, subject_id, trimester_id, date, requestId) {
		const query = `INSERT INTO asignation (room_id, subject_id, trimester_id, date) VALUES ('${room}','${subject_id}','${trimester_id}','${date}') RETURNING id`;
		...
		for (let index = 0; index < request_schedule.rowCount; index++) {
			...
			const values = [id, element.week, element.day, element.hour];
			const query1 = 'INSERT INTO asig_schedule (asignation_id, week, day, hour) VALUES ($1,$2,$3,$4)';
	...
	async deleteRequest(id) {
		...
		const queryDeleteSchedule = `DELETE FROM reservation_request_schedule WHERE reservation_request_id = ${id}`;
		const queryDeleteRequest = `DELETE FROM reservation_request WHERE id = ${id}`;
